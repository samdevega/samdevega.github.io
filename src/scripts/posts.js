...
export const translatePosts = async (lang) => {
  ...
  return sortedPosts.filter(post => post.id.includes(`${lang}/`)).map((post, index, posts) => {
    post.data.minRead = Math.ceil(post.body?.replaceAll(/<table>(.|\n)*<\/table>/g, '').split(' ').length / 200) || 0
    post.data.next = posts[index - 1]?.id.slice(3)
    post.data.previous = posts[index + 1]?.id.slice(3)
