...
function sortUsersByName(users: User[], order: Order = 'ascending'): User[] {
  ...
  sorted.sort((a, b) => {
    return a.name > b.name ? sortValues[order][0] : a.name < b.name ? sortValues[order][1] : 0
  })
  return sorted
}
```
...
function sortUsersBy(users: User[], field: keyof User, order: Order = 'ascending'): User[] {
  ...
  sorted.sort((a, b) => {
    return a[field] > b[field] ? sortValues[order][0] : a[field] < b[field] ? sortValues[order][1] : 0
  })
  return sorted
}
```
